...
import com.cleveroad.sample.adapter.TestImageView;
...
public class TableLayoutFragment
    ...
    @Override
    public void onItemClickTest(int row, int column, ViewHolder viewHolder) {

        TestImageView testImageView = com.cleveroad.sample.adapter.TestImageView.newInstance(row);
       testImageView.setmTableDataSource(mCsvFileDataSource);
        testImageView.setDataRow(row);
        testImageView.setDataColumn(column);
//        testImageView.setViewHolder((ViewHolderImpl) viewHolder);

        DetailsViewHolder detailsViewHolder = new DetailsViewHolder(viewHolder.getItemView());

        String itemData = mCsvFileDataSource.getItemData(row, column);
        itemData.trim();
        ImageView itemImage = (ImageView) viewHolder.getItemView().findViewById(R.id.ivImage);

        ViewCompat.setTransitionName(viewHolder.getItemView().findViewById(R.id.ivImage), "detailsImage");

        detailsViewHolder.getImage();

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            testImageView.setSharedElementEnterTransition(new DetailsTransition());
            testImageView.setEnterTransition(new Fade());
            setExitTransition(new Fade());
            testImageView.setSharedElementReturnTransition(new DetailsTransition());
        }

        //Intent intent = new Intent(TableLayoutFragment.this.getActivity(), TestImageView.class);
        //TableLayoutFragment.this.startActivity(intent);
        //intent.putExtra("view_holder", (Serializable) viewHolder);
        getActivity().getSupportFragmentManager()
                .beginTransaction()
                .addSharedElement(itemImage, ViewCompat.getTransitionName(itemImage))
                .replace(R.id.container, testImageView)
                .addToBackStack(TAG)
                .commit();
    }
...
/*
    public class TestViewHolderFragment extends ViewHolderImpl {
        TextView tvText;
        ImageView ivImage;

        private TestViewHolderFragment(@NonNull View itemView) {
            super(itemView);
